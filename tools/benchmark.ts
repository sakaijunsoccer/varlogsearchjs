...
const benchmarkEventLog = async (filename: string, keyword: string, limit: number) => {

    const buffer_size = 16;
    const start = (new Date()).getTime() / 1000;
    const eventLog = new EventLog(filename, buffer_size);
    const results = await eventLog.search([keyword], limit);
    ...
    const results2 = await eventLog2.search([keyword], limit);
